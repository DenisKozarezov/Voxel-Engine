...
namespace VoxelEngine::input
{
    ...
    public:
        ...
        virtual FORCE_INLINE const size_t& getHashCode() const = 0;
        virtual FORCE_INLINE void invoke(input::Event& arg) = 0;
        virtual FORCE_INLINE std::future<void> invoke_async(input::Event& arg, const std::launch& launch) = 0;
    };
    ...
    public:
        ...
        { }
        EventHandler(EventHandler&& src) noexcept : m_cbFunc(std::move(src.m_cbFunc)), m_eventType(src.m_eventType)
        { }
        ~EventHandler() noexcept = default;

        FORCE_INLINE bool operator==(const EventHandler& rhs) noexcept
        {
        ...
        FORCE_INLINE void operator()(TEvent& arg) const
        {
            invoke(arg);
        }

        FORCE_INLINE const size_t& getHashCode() const override { return m_eventType; }

        FORCE_INLINE void invoke(input::Event& arg) override
        {            
            this->invoke(static_cast<TEvent&>(arg));
        }
        FORCE_INLINE std::future<void> invoke_async(input::Event& arg, const std::launch& launch) override
        {
            return this->invoke_async(static_cast<TEvent&>(arg), launch);
        }
        FORCE_INLINE void invoke(TEvent& arg)
        { 
            this->m_cbFunc(std::forward<TEvent&>(arg));
        }
        FORCE_INLINE std::future<void> invoke_async(TEvent& arg, const std::launch& launch)
        {
